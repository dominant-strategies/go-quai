...
func (s *Service) assembleBlockDetailStats(block *types.WorkObject) *blockDetailStats {
	...
	_, blockOrder, err := s.backend.(fullNodeBackend).CalcOrder(block)
	if err != nil {
		s.backend.Logger().WithField("err", err).Error("Failed to calculate order of the block")
	}
	var inboundEtxs types.Transactions
	conversionAmount := big.NewInt(0)
	if blockOrder == common.PRIME_CTX {
		inboundEtxs = rawdb.ReadInboundEtxs(s.backend.Database(), block.Hash())
		if inboundEtxs != nil {
			conversionAmount = misc.ComputeConversionAmountInQuai(block, inboundEtxs)
		}
	} else {
		inboundEtxs = rawdb.ReadInboundEtxs(s.backend.Database(), primeTerminus.Hash())
		if inboundEtxs != nil {
			conversionAmount = misc.ComputeConversionAmountInQuai(primeTerminus, inboundEtxs)
		}
	}

	slip := big.NewInt(0)
	if conversionAmount.Cmp(common.Big0) != 0 {
		// As running the inboundEtxs processing is expensive, the slip is
		// calculated on the parent flow and k quai discount values
		conversionAmountAfterKQuaiDiscount := new(big.Int).Mul(conversionAmount, new(big.Int).SetInt64(params.KQuaiDiscountMultiplier-block.KQuaiDiscount().Int64()))
		conversionAmountAfterKQuaiDiscount = new(big.Int).Div(conversionAmountAfterKQuaiDiscount, new(big.Int).SetInt64(params.KQuaiDiscountMultiplier))
		conversionAmountAfterFlowDiscount := misc.ApplyCubicDiscount(conversionAmountAfterKQuaiDiscount, block.KQuaiDiscount())
		conversionAmountAfterFlowDiscountInt, _ := conversionAmountAfterFlowDiscount.Int(nil)

		slip = new(big.Int).Mul(conversionAmountAfterFlowDiscountInt, common.Big100)
		slip = new(big.Int).Div(slip, conversionAmount)
	}
