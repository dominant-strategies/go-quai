...
func (context *chainContext) AddToCalcOrderCache(hash common.Hash, order int, intrinsicS *big.Int) {
	context.api.backend.AddToCalcOrderCache(hash, order, intrinsicS)
}

func (context *chainContext) CalcMaxBaseFee(block *types.WorkObject) (*big.Int, error) {
	return context.api.backend.CalcMaxBaseFee(block)
}
