...
func (hc *HeaderChain) FetchUtxosMain(view *types.UtxoViewpoint, outpoints []types.OutPoint) error {
	...
	for i := range outpoints {
		entry := hc.GetUtxo(outpoints[i].TxHash)
		if entry == nil {
			return nil
		}
...
func (hc *HeaderChain) VerifyUTXOsForTx(tx *types.Transaction) error {
	...
	for _, txIn := range tx.TxIn() {
		utxo := hc.GetUtxo(txIn.PreviousOutPoint.TxHash)
		if utxo == nil {
...
func (hc *HeaderChain) WriteUtxoViewpoint(view *types.UtxoViewpoint) error {
	for outpoint, entry := range view.Entries {
		...
		if entry.IsSpent() {
			rawdb.DeleteUtxo(hc.bc.db, outpoint.TxHash)
			continue
		}

		fmt.Println("write utxo", outpoint.TxHash.Hex())
		rawdb.WriteUtxo(hc.bc.db, outpoint.TxHash, entry)
	}
...
func createCoinbaseTx(parentHash common.Hash, addr common.Address) (*types.Transaction, error) {
	in := types.TxIn{
