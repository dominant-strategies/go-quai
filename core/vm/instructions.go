...
func opConvert(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	// Fail if ETX gas price or tip are not valid (needs refactoring)
	if err := interpreter.evm.ValidateETXGasPriceAndTip(scope.Contract.Caller(), toAddr, interpreter.evm.GasPrice, interpreter.evm.TXGasTip); err != nil {
		temp.Clear()
		stack.push(&temp)
		fmt.Printf("%x opConvert error: %s\n", scope.Contract.self.Address(), err.Error())
		return nil, nil // following opCall protocol
