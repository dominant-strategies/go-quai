...
ifeq (,$(wildcard nodelogs))
	...
	@nohup $(MINING_BASE_COMMAND) --miner.etherbase $(REGION_0_COINBASE) --http.addr $(HTTP_ADDR) --http.api $(HTTP_API) --ws.addr $(WS_ADDR) --ws.api $(WS_API)  --port $(REGION_0_PORT_TCP) --http.port $(REGION_0_PORT_HTTP) --ws.port $(REGION_0_PORT_WS) --dom.url $(REGION_0_DOM_URL):$(PRIME_PORT_WS) --sub.urls $(REGION_0_SUB_URLS) --region 0 --quaistats ${STATS_NAME}:${STATS_PASS}@${REGION_0_STATS_HOST} >> nodelogs/region-0.log 2>&1 &
	...
	@nohup $(MINING_BASE_COMMAND) --miner.etherbase $(ZONE_2_0_COINBASE) --http.addr $(HTTP_ADDR) --http.api $(HTTP_API) --ws.addr $(WS_ADDR) --ws.api $(WS_API)  --port $(ZONE_2_0_PORT_TCP) --http.port $(ZONE_2_0_PORT_HTTP) --ws.port $(ZONE_2_0_PORT_WS) --dom.url $(ZONE_2_0_DOM_URL):$(REGION_2_PORT_WS) --region 2 --zone 0 --quaistats ${STATS_NAME}:${STATS_PASS}@${ZONE_2_0_STATS_HOST} >> nodelogs/zone-2-0.log 2>&1 &
