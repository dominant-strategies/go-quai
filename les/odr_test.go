...
func odrAccounts(ctx context.Context, db ethdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
	...
	for _, addr := range acc {
		if bc != nil {
			...
			st, err = state.New(header.Root[types.QuaiNetworkContext], state.NewDatabase(db), nil)
		} else {
...
func odrContractCall(ctx context.Context, db ethdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash common.Hash) []byte {
	...
	for i := 0; i < 3; i++ {
		...
		if bc != nil {
			header := bc.GetHeaderByHash(bhash)
			statedb, err := state.New(header.Root[types.QuaiNetworkContext], state.NewDatabase(db), nil)
...
func testOdr(t *testing.T, protocol int, expFail uint64, checkCached bool, fn odrTestFn) {
	...
	if clientHead.Number[types.QuaiNetworkContext].Uint64() != 4 {
		t.Fatalf("Failed to sync the chain with server, head: %v", clientHead.Number[types.QuaiNetworkContext].Uint64())
	}
	...
	test := func(expFail uint64) {
		...
		for i := uint64(0); i <= server.handler.blockchain.CurrentHeader().Number[types.QuaiNetworkContext].Uint64(); i++ {
