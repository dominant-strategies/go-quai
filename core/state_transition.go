...
var (
	kQuaiSettingAddress = common.HexToAddress("0x", common.Location{0, 0})
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if msg.From().Equal(kQuaiSettingAddress) && st.evm.Context.BlockNumber.Uint64() < params.BlocksPerYear {
		if !st.msg.IsETX() && !contractCreation && bytes.Equal(st.data[:updateKQuaiLen], updateKQuai) {
		...
		} else if !st.msg.IsETX() && !contractCreation && bytes.Equal(st.data[:freezeKQuaiLen], freezeKQuai) {
			err := st.state.FreezeKQuai()
			if err != nil {
				return nil, fmt.Errorf("unable to freeze k quai: %v", err)
			}
		} else if !st.msg.IsETX() && !contractCreation && bytes.Equal(st.data[:unfreezeKQuaiLen], unfreezeKQuai) {
