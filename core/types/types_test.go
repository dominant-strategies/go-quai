...
func benchRLP(b *testing.B, encode bool) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			"legacy-header",
			&Header{
				Difficulty: []*big.Int{big.NewInt(10000000000)},
				Number:     []*big.Int{big.NewInt(1000)},
				GasLimit:   []uint64{8_000_000},
				GasUsed:    []uint64{8_000_000},
				Time:       555,
				Extra:      [][]byte{make([]byte, 32)},
			},
		},
		{
			"london-header",
			&Header{
				...
				Time:       555,
				Extra:      [][]byte{make([]byte, 32)},
				BaseFee:    []*big.Int{big.NewInt(10000000000)},
			},
		},
		{
			"receipt-for-storage",
