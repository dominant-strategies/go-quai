...
func (p *Peer) Head() (hash common.Hash, entropy *big.Int, receivedAt time.Time) {
	p.lock.RLock()
	defer p.lock.RUnlock()

	copy(hash[:], p.head[:])
	return hash, p.entropy, p.receivedHeadAt
}

// SetHead updates the head hash and head number of the peer.
func (p *Peer) SetHead(hash common.Hash, entropy *big.Int, receivedAt time.Time) {
	p.lock.Lock()
	defer p.lock.Unlock()
