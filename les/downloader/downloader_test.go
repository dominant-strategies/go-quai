...
func (dl *downloadTester) InsertHeaderChain(headers []*types.Header, checkFreq int) (i int, err error) {
	...
	if dl.getHeaderByHash(headers[0].ParentHash[0]) == nil {
	...
	for i, header := range headers {
		...
		if dl.getHeaderByHash(header.ParentHash[0]) == nil {
		...
		td := dl.getTd(header.ParentHash[0])
		dl.ownChainTd[hash] = new(big.Int).Add(td, header.Difficulty[0])
	}
...
func (dl *downloadTester) SetHead(head uint64) error {
	...
	for h, header := range dl.ownHeaders {
		if header.Number[0].Uint64() == head {
			hash = h
		}
	}
...
func testInvalidHeaderRollback(t *testing.T, protocol uint, mode SyncMode) {
	...
	if head := tester.CurrentHeader().Number[0].Int64(); int(head) > MaxHeaderFetch {
	...
	if head := tester.CurrentHeader().Number[0].Int64(); int(head) > 2*fsHeaderSafetyNet+MaxHeaderFetch {
