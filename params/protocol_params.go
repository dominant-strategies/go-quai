...
func init() {
	...
	// These lockup bytes are divided by 10,000 while calculating the reward
	// The first value represents the interest rate for the first year, the second
	// value represents the terminal rate
	LockupByteToRewardsMultiple[1] = []uint64{3500, 218}
	LockupByteToRewardsMultiple[2] = []uint64{10000, 625}
	LockupByteToRewardsMultiple[3] = []uint64{25000, 1562}
}
...
func CalculateLockupByteRewardsMultiple(lockupByte uint8, blockNumber uint64) (*big.Int, error) {
	...
	if year == 0 {
		...
		// y = mx + c
		c := LockupByteToRewardsMultiple[lockupByte][0]
		m := (LockupByteToRewardsMultiple[lockupByte][0] - LockupByteToRewardsMultiple[lockupByte][1]) / 4 * BlocksPerYear
		x := blockNumber - BlocksPerYear // subtracting a year because the discount starts after the first year
		rewardsMultiple := m*x + c
		return big.NewInt(int64(rewardsMultiple)), nil
	}
}
...
func CalculateCoinbaseValueWithLockup(value *big.Int, lockupByte uint8, blockNumber uint64) *big.Int {
	if lockupByte == 0 {
		...
		rewardValue = new(big.Int).Div(rewardValue, big.NewInt(10000))
		return new(big.Int).Add(value, rewardValue)
	}
}
