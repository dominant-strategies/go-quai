...
func (h *clientHandler) handle(p *serverPeer, noInitAnnounce bool) error {
	...
	forkid := forkid.NewID(h.backend.blockchain.Config(), h.backend.genesis, h.backend.blockchain.CurrentHeader().Number[types.QuaiNetworkContext].Uint64())
...
func (d *downloaderPeerNotify) registerPeer(p *serverPeer) {
	...
	h.downloader.RegisterLightPeer(p.id, eth.QUAI66, pc)
}
