...
func (t *Tracker) Track(peer string, version uint, reqCode uint64, resCode uint64, id uint64) {
	...
	// metrics.GetOrRegisterCounter(g, nil).Inc(1)
...
func (t *Tracker) clean() {
	...
	for t.expire.Len() > 0 {
		...
		// metrics.GetOrRegisterCounter(g, nil).Dec(1)
...
func (t *Tracker) Fulfil(peer string, version uint, code uint64, id uint64) {
	...
	// metrics.GetOrRegisterHistogram(h, nil, metrics.NewExpDecaySample(1028, 0.015)).Update(time.Since(req.time).Microseconds())
}
