...
func (*dummyStatedb) GetBalance(addr common.InternalAddress) *big.Int { return new(big.Int) }
...
func TestTracer(t *testing.T) {
	execTracer := func(code string) ([]byte, string) {
		t.Helper()
		tracer, err := New(code, new(Context), log.NewLogger("testlog", "error", 0), common.LocationFromAddressBytes([]byte{0, 0}))
		if err != nil {
			t.Fatal(err)
		}
...
func TestHalt(t *testing.T) {
	...
	tracer, err := New("{step: function() { while(1); }, result: function() { return null; }}", new(Context), log.NewLogger("testlog", "error", 0), common.LocationFromAddressBytes([]byte{0, 0}))
	if err != nil {
		t.Fatal(err)
	}
	go func() {
...
func TestHaltBetweenSteps(t *testing.T) {
	tracer, err := New("{step: function() {}, fault: function() {}, result: function() { return null; }}", new(Context), log.NewLogger("testlog", "error", 0), common.LocationFromAddressBytes([]byte{0, 0}))
	if err != nil {
		t.Fatal(err)
	}
	...
	tracer.CaptureState(env, 0, 0, 0, 0, scope, nil, 0, nil, common.LocationFromAddressBytes([]byte{0, 0}))
	timeout := errors.New("stahp")
	tracer.Stop(timeout)
	tracer.CaptureState(env, 0, 0, 0, 0, scope, nil, 0, nil, common.LocationFromAddressBytes([]byte{0, 0}))
