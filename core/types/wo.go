...
func (wo *WorkObject) SetParentHash(val common.Hash, nodeCtx int) {
	if nodeCtx == common.ZONE_CTX {
		...
		wo.Body().Header().ParentHash(nodeCtx)
	}
}
...
func (wo *WorkObject) WithBody(header *Header, txs []*Transaction, etxs []*Transaction, uncles []*WorkObject, manifest BlockManifest, interlinkHashes common.Hashes) *WorkObject {
	woBody := &WorkObjectBody{
		header:          CopyHeader(header),
		transactions:    make([]*Transaction, len(txs)),
		uncles:          make([]*WorkObject, len(uncles)),
	...
	for i := range uncles {
		woBody.uncles[i] = CopyWorkObject(uncles[i])
	}
...
func NewWorkObjectBody(header *Header, txs []*Transaction, etxs []*Transaction, uncles []*WorkObject, manifest BlockManifest, receipts []*Receipt, hasher TrieHasher, nodeCtx int) *WorkObjectBody {
	b := &WorkObjectBody{}
	...
	if len(uncles) == 0 {
		...
		b.uncles = make([]*WorkObject, len(uncles))
		for i := range uncles {
			b.uncles[i] = CopyWorkObject(uncles[i])
		}
	}
