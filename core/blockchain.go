...
func (bc *BlockChain) PCRC(header *types.Header) (common.Hash, error) {
	...
	if headerOrder <= params.REGION {
		...
		if RTZ.Hash() != RTR.Hash() {
			...
			retErr = append(retErr, "there exists a region (RTZ) twist")
		}
	}
	...
	if headerOrder == params.PRIME {
		...
		if PTR.Hash() != PTP.Hash() {
			log.Info("Error in PCRC", "PTR", PTR.Hash(), "num", PTR.Number, "PTP", PTP.Hash(), "num", PTP.Number)
			...
			retErr = append(retErr, "there exists a prime (PTP) twist")
		}

		if PTZ.Hash() != PTR.Hash() {
			log.Info("Error in PCRC", "PTZ", PTZ.Hash(), "num", PTZ.Number, "PTR", PTR.Hash(), "num", PTR.Number)
			if types.QuaiNetworkContext == params.PRIME {
			...
			retErr = append(retErr, "there exists a prime (PTR) twist")
		}
	}

	if len(retErr) > 0 {
		return common.Hash{}, errors.New("there exists a twist")
	}
