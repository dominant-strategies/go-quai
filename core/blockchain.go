...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, domClientUrl string, subClientUrls []string, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	var err error
	bc.hc, err = NewHeaderChain(db, chainConfig, cacheConfig, engine, domClientUrl, subClientUrls, bc.insertStopped)
	if err != nil {
		return nil, err
	}
...
func (bc *BlockChain) SetHeadBeyondRoot(head uint64, root common.Hash) (uint64, error) {
	...
	bc.hc.externalBlockQueue.Purge()
...
func (bc *BlockChain) Stop() {
	...
	bc.hc.externalBlocks.SaveToFileConcurrent(bc.cacheConfig.ExternalBlockJournal, runtime.GOMAXPROCS(0))
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, linkExtBlocks []*types.ExternalBlock) error {
	...
	externTd, err := bc.hc.CalcTd(block.Header())
	if err != nil {
		return err
	}
...
func (bc *BlockChain) AddExternalBlocks(blocks []*types.ExternalBlock) error {
	return bc.hc.AddExternalBlocks(blocks)
}
...
func (bc *BlockChain) AddExternalBlock(block *types.ExternalBlock) error {
	return bc.hc.AddExternalBlock(block)
}
...
func (bc *BlockChain) DomReorgNeeded(header *types.Header) (bool, error) {
	...
	domStatus := bc.hc.domClient.GetBlockStatus(context.Background(), terminalHeader)
...
func (bc *BlockChain) GetExternalBlock(hash common.Hash, location []byte, context uint64) (*types.ExternalBlock, error) {
	return bc.hc.GetExternalBlock(hash, location, context)
}
...
func (bc *BlockChain) GetExternalBlockByHashAndContext(hash common.Hash, context int) (*types.ExternalBlock, error) {
	return bc.hc.GetExternalBlockByHashAndContext(hash, context)
}
...
func (bc *BlockChain) QueueAndRetrieveExtBlocks(externalBlocks []*types.ExternalBlock, header *types.Header) []*types.ExternalBlock {
	for _, block := range externalBlocks {
		bc.hc.externalBlockQueue.Add(block.Hash(), block)
	}
	resultBlocks := make([]*types.ExternalBlock, 0)
	gasUsed := 0
	for {
		key, result, ok := bc.hc.externalBlockQueue.GetOldest()
		if !ok || result == nil {
			break
		}
		...
		bc.hc.externalBlockQueue.Remove(key)
...
func (bc *BlockChain) PCRC(header *types.Header, headerOrder int) (types.PCRCTermini, error) {
	return bc.hc.PCRC(header, headerOrder)
}
...
func (bc *BlockChain) RollbackToCanonical(header *types.Header) error {
	...
	for {
		status := bc.hc.domClient.GetBlockStatus(context.Background(), header)
...
func (bc *BlockChain) subscribeDomHead() {
	...
	sub, err := bc.hc.domClient.SubscribeNewHead(context.Background(), newHeadChannel)
	if err != nil {
	...
	for {
		...
		case newHead := <-newHeadChannel:
			...
			if !bytes.Equal(newHead.Location, bc.chainConfig.Location) {
				newHead, err = bc.hc.domClient.GetAncestorByLocation(context.Background(), newHead.Hash(), bc.chainConfig.Location)
				if err != nil {
					fmt.Println(err)
					continue
				}
			...
			extBlockSet, err := bc.hc.domClient.GetSubordinateSet(context.Background(), bc.CurrentBlock().Hash(), bc.chainConfig.Location)
			if err != nil {
			...
			for _, newHash := range extBlockSet {
				extBlock, err := bc.hc.domClient.GetExternalBlockByHashAndContext(context.Background(), newHash, types.QuaiNetworkContext)
				if err != nil {
...
func (bc *BlockChain) SubscribeMissingExternalBlockEvent(ch chan<- MissingExternalBlock) event.Subscription {
	return bc.scope.Track(bc.hc.missingExternalBlockFeed.Subscribe(ch))
}
...
func (bc *BlockChain) CheckContext(number int) error {
	return bc.hc.CheckContext(number)
}
...
func (bc *BlockChain) CheckLocationRange(location []byte) error {
	return bc.hc.CheckLocationRange(location)
}
