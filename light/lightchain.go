...
func NewLightChain(odr OdrBackend, config *params.ChainConfig, engine consensus.Engine, checkpoint *params.TrustedCheckpoint) (*LightChain, error) {
	...
	var err error
	bc.hc, err = core.NewHeaderChain(odr.Database(), bc.engine, config, nil, vm.Config{})
	if err != nil {
		return nil, err
	}
...
func (lc *LightChain) ResetWithGenesisBlock(genesis *types.Block) {
	...
	rawdb.WriteTd(batch, genesis.Hash(), genesis.NumberU64(), genesis.Header().Difficulty[types.QuaiNetworkContext])
