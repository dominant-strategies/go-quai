...
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, eth.QUAI66) }
...
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, eth.QUAI66) }
...
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, eth.QUAI66) }
...
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, eth.QUAI66) }
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := eth.NewPeer(eth.QUAI66, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := eth.NewPeer(eth.QUAI66, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	if checkpoint {
		...
		if query.Origin.Number != response.Number[types.QuaiNetworkContext].Uint64() || query.Amount != 1 || query.Skip != 0 || query.Reverse {
			t.Fatalf("challenge mismatch: have [%d, %d, %d, %v] want [%d, %d, %d, %v]",
				query.Origin.Number, query.Amount, query.Skip, query.Reverse,
				response.Number[types.QuaiNetworkContext].Uint64(), 1, 0, false)
		}
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := eth.NewPeer(eth.QUAI66, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := eth.NewPeer(eth.QUAI66, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, eth.QUAI66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	for _, header := range []*types.Header{malformedUncles, malformedTransactions, malformedEverything} {
		...
		if err := src.SendNewBlock(block, []*big.Int{big.NewInt(131136), big.NewInt(131136), big.NewInt(131136)}, []*types.ExternalBlock{types.NewExternalBlockWithHeader(header)}); err != nil {
