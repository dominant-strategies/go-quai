...
func (sl *Slice) Append(header *types.Header, domPendingHeader *types.Header, domTerminus common.Hash, domOrigin bool, newInboundEtxs types.Transactions) (types.Transactions, bool, error) {
	...
	if nodeCtx == common.ZONE_CTX {
		subReorg = sl.pickPhHead(pendingHeaderWithTermini, &appendFinished, oldBestPhEntropy)
	}
	if subReorg {
		if &appendFinished != nil { // If appendTime is nil, it means this was called from updatePhCacheFromDom
...
func (sl *Slice) readPhCache(hash common.Hash) (types.PendingHeader, bool) {
	...
	log.Info("readPhCache", "lock time:", common.PrettyDuration(time.Since(start)))
	if exists {
...
func (sl *Slice) writePhCache(hash common.Hash, pendingHeader types.PendingHeader) {
	...
	log.Info("writePhCache", "lock time:", common.PrettyDuration(time.Since(start)))
}
...
func (sl *Slice) generateSlicePendingHeader(block *types.Block, newTermini []common.Hash, domPendingHeader *types.Header, domOrigin bool, fill bool) (types.PendingHeader, error) {
	...
	copyTermini := make([]common.Hash, 4)
	for i, termini := range newTermini {
		copyTermini[i] = termini
	}

	// Combine subordinates pending header with local pending header
	pendingHeaderWithTermini := sl.computePendingHeader(types.PendingHeader{Header: localPendingHeader, Termini: copyTermini}, domPendingHeader, domOrigin)
...
func (sl *Slice) computePendingHeader(localPendingHeaderWithTermini types.PendingHeader, domPendingHeader *types.Header, domOrigin bool) types.PendingHeader {
	...
	log.Info("computePendingHeader:", "hash:", hash, "pendingHeader:", cachedPendingHeaderWithTermini, "termini:", cachedPendingHeaderWithTermini.Termini)
...
func (sl *Slice) updatePhCacheFromDom(pendingHeader types.PendingHeader, terminiIndex int, indices []int) error {
	...
	if exists {
		...
		sl.pickPhHead(types.PendingHeader{Header: combinedPendingHeader, Termini: localPendingHeader.Termini}, nil, oldBestPhEntropy)
		return nil
	}
...
func (sl *Slice) updatePhCache(pendingHeaderWithTermini types.PendingHeader, inSlice bool) {
	...
	if exist {
		...
		if inSlice && newPhEntropy.Cmp(oldPh.Header.CalcPhS()) >= 0 {
			...
			log.Info("PhCache update inSlice ", "Ph Number:", deepCopyPendingHeaderWithTermini.Header.NumberArray(), "Termini:", pendingHeaderWithTermini.Termini[c_terminusIndex])
			// if we are not inslice we are getting a coordinate update from dom which has already been decided by a coord
		} else if !inSlice {
			sl.writePhCache(pendingHeaderWithTermini.Termini[c_terminusIndex], deepCopyPendingHeaderWithTermini)
			log.Info("PhCache update coord ", "Ph Number:", deepCopyPendingHeaderWithTermini.Header.NumberArray(), "Termini:", pendingHeaderWithTermini.Termini[c_terminusIndex])
		}
	} else {
		if inSlice {
			sl.writePhCache(pendingHeaderWithTermini.Termini[c_terminusIndex], deepCopyPendingHeaderWithTermini)
...
func (sl *Slice) pickPhHead(pendingHeaderWithTermini types.PendingHeader, appendTime *time.Duration, oldBestPhEntropy *big.Int) bool {
	newPhEntropy := pendingHeaderWithTermini.Header.CalcPhS()
	// Pick a phCache Head
