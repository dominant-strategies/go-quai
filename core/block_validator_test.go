...
func TestHeaderVerification(t *testing.T) {
	...
	var (
		...
		blocks, _ = GenerateChain(params.TestChainConfig, genesis, blake3.NewFaker(), testdb, 8, nil)
	)
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, blake3.NewFaker(), vm.Config{}, nil, nil)
	defer chain.Stop()
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	var (
		...
		gspec     = &Genesis{Config: params.TestChainConfig}
		genesis   = gspec.MustCommit(testdb)
		blocks, _ = GenerateChain(params.TestChainConfig, genesis, blake3.NewFaker(), testdb, 1024, nil)
	)
	headers := make([]*types.Header, len(blocks))
	seals := make([]bool, len(blocks))
...
func TestCalcGasLimit(t *testing.T) {
	for i, tc := range []struct {
		...
		// Increase
		if have, want := CalcGasLimit(tc.pGasLimit, 0, 0), tc.max; have != want {
			t.Errorf("test %d: have %d want <%d", i, have, want)
		}
		// Decrease
		if have, want := CalcGasLimit(tc.pGasLimit, 0, 0), tc.min; have != want {
			t.Errorf("test %d: have %d want >%d", i, have, want)
		}
		// Small decrease
		if have, want := CalcGasLimit(tc.pGasLimit, 0, 0), tc.pGasLimit-1; have != want {
			t.Errorf("test %d: have %d want %d", i, have, want)
		}
		// Small increase
		if have, want := CalcGasLimit(tc.pGasLimit, 0, 0), tc.pGasLimit+1; have != want {
		...
		// No change
		if have, want := CalcGasLimit(tc.pGasLimit, 0, 0), tc.pGasLimit; have != want {
			t.Errorf("test %d: have %d want %d", i, have, want)
		}
	}
}
