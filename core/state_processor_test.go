...
func TestStateProcessorErrors(t *testing.T) {
	...
	{ // Tests against a 'recent' chain definition
		...
		for i, tt := range []struct {
			...
			block := GenerateBadBlock(genesis, blake3.NewFaker(), tt.txs, gspec.Config, blockchain)
...
func GenerateBadBlock(parent *types.Block, engine consensus.Engine, txs types.Transactions, config *params.ChainConfig, chain *BlockChain) *types.Block {
	hash := common.Hash{}
	header := &types.Header{
		Time:       parent.Time() + 10,
	...
	header.BaseFee[types.QuaiNetworkContext] = misc.CalcBaseFee(config, parent.Header(), chain.GetHeaderByNumber, chain.GetUnclesInChain, chain.GetGasUsedInChain)
