...
func handleNewBlock(backend Backend, msg Decoder, peer *Peer) error {
	...
	if hash := types.DeriveSha(ann.Block.Transactions(), trie.NewStackTrie(nil)); hash != ann.Block.TxHash() {
		log.Warn("Propagated block has invalid body(tx)", "have", hash, "exp", ann.Block.TxHash())
		return nil // TODO(karalabe): return error eventually, but wait a few releases
	}
	if hash := types.DeriveSha(ann.Block.ExtTransactions(), trie.NewStackTrie(nil)); hash != ann.Block.EtxHash() {
		log.Warn("Propagated block has invalid body(etx)", "have", hash, "exp", ann.Block.EtxHash())
		return nil // TODO(karalabe): return error eventually, but wait a few releases
	}
	...
	if nodeCtx < common.ZONE_CTX {
		if hash := types.DeriveSha(ann.Block.SubManifest(), trie.NewStackTrie(nil)); hash != ann.Block.ManifestHash(nodeCtx+1) {
			log.Warn("Propagated block has invalid body(submanifest)", "have", hash, "exp", ann.Block.ManifestHash())
			return nil // TODO(karalabe): return error eventually, but wait a few releases
		}
	}
