...
func NewTxPool(config *params.ChainConfig, chain *LightChain, relay TxRelayBackend) *TxPool {
	pool := &TxPool{
		...
		clearIdx:    chain.CurrentHeader().Number[types.QuaiNetworkContext].Uint64(),
	}
...
func (pool *TxPool) reorgOnNewHead(ctx context.Context, newHeader *types.Header) (txStateChanges, error) {
	...
	for oldh.Hash() != newh.Hash() {
		if oldh.Number[types.QuaiNetworkContext].Uint64() >= newh.Number[types.QuaiNetworkContext].Uint64() {
			...
			oldh = pool.chain.GetHeader(oldh.ParentHash[types.QuaiNetworkContext], oldh.Number[types.QuaiNetworkContext].Uint64()-1)
		}
		if oldh.Number[types.QuaiNetworkContext].Uint64() < newh.Number[types.QuaiNetworkContext].Uint64() {
			newHashes = append(newHashes, newh.Hash())
			newh = pool.chain.GetHeader(newh.ParentHash[types.QuaiNetworkContext], newh.Number[types.QuaiNetworkContext].Uint64()-1)
			if newh == nil {
	...
	if oldh.Number[types.QuaiNetworkContext].Uint64() < pool.clearIdx {
		pool.clearIdx = oldh.Number[types.QuaiNetworkContext].Uint64()
	}
	...
	for i := len(newHashes) - 1; i >= 0; i-- {
		hash := newHashes[i]
		if err := pool.checkMinedTxs(ctx, hash, newHeader.Number[types.QuaiNetworkContext].Uint64()-uint64(i), txc); err != nil {
			return txc, err
		}
		pool.head = hash
	}
	...
	if idx := newHeader.Number[types.QuaiNetworkContext].Uint64(); idx > pool.clearIdx+txPermanent {
...
func (pool *TxPool) setNewHead(head *types.Header) {
	...
	next := new(big.Int).Add(head.Number[types.QuaiNetworkContext], big.NewInt(1))
...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	if header.GasLimit[types.QuaiNetworkContext] < tx.Gas() {
