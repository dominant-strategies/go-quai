...
func makeChain(n int, seed byte, parent *types.Block, empty bool) ([]*types.Block, []types.Receipts) {
	blocks, receipts := core.GenerateChain(params.TestChainConfig, parent, blake3.NewFaker(), testdb, n, func(i int, block *core.BlockGen) {
...
func TestBasics(t *testing.T) {
	...
	// queue that a certain peer will deliver them for us
	{
		...
		if got, exp := fetchReq.Headers[0].Number[types.QuaiNetworkContext].Uint64(), uint64(1); got != exp {
...
func XTestDelivery(t *testing.T) {
	...
	go func() {
		...
		for {
			...
			//	l, hdrs[0].Number[types.QuaiNetworkContext].Uint64(), hdrs[len(hdrs)-1].Number[types.QuaiNetworkContext].Uint64())
	...
	go func() {
		...
		for {
			...
			world.forget(res[len(res)-1].Header.Number[types.QuaiNetworkContext].Uint64())
		...
		for {
			...
			if f != nil {
				...
				for _, hdr := range f.Headers[0 : len(f.Headers)-numToSkip] {
					txs = append(txs, world.getTransactions(hdr.Number[types.QuaiNetworkContext].Uint64()))
		...
		for {
			...
			if f != nil {
				...
				for _, hdr := range f.Headers {
					rcs = append(rcs, world.getReceipts(hdr.Number[types.QuaiNetworkContext].Uint64()))
				}
	...
	go func() {
		...
		for {
			...
			fmt.Printf("world block tip is %d\n",
				world.chain[len(world.chain)-1].Header().Number[types.QuaiNetworkContext].Uint64())
			fmt.Println(q.Stats())
		}
	}()
	wg.Wait()
...
func (n *network) getReceipts(blocknum uint64) types.Receipts {
	index := blocknum - uint64(n.offset)
	if got := n.chain[index].Header().Number[types.QuaiNetworkContext].Uint64(); got != blocknum {
