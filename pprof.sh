...
do
    ...
    go tool pprof -svg -alloc_space http://localhost:$port/debug/pprof/heap > "$name"_"$port"_heap_alloc.svg
    go tool pprof -svg -inuse_space http://localhost:$port/debug/pprof/heap > "$name"_"$port"_heap_inuse.svg
done
