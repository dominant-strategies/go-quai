...
func newCanonical(engine consensus.Engine, n int, full bool) (ethdb.Database, *BlockChain, error) {
	var (
		...
		genesis = (&Genesis{BaseFee: []*big.Int{big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee)}}).MustCommit(db)
	)
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 []*big.Int)) {
	...
	db, blockchain2, err := newCanonical(blake3.NewFaker(), i, full)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.CurrentBlock(), n, blake3.NewFaker(), db, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.CurrentHeader(), n, blake3.NewFaker(), db, forkSeed)
...
func testBlockChainImport(chain types.Blocks, blockchain *BlockChain) error {
	for _, block := range chain {
		...
		rawdb.WriteTd(blockchain.db, block.Hash(), block.NumberU64(), []*big.Int{new(big.Int).Add(block.Difficulty(), blockchain.GetTdByHash(block.ParentHash())[0]), new(big.Int).Add(block.Difficulty(), blockchain.GetTdByHash(block.ParentHash())[1]), new(big.Int).Add(block.Difficulty(), blockchain.GetTdByHash(block.ParentHash())[2])})
...
func testHeaderChainImport(chain []*types.Header, blockchain *BlockChain) error {
	for _, header := range chain {
		...
		blockchain.chainmu.Lock()
		rawdb.WriteTd(blockchain.db, header.Hash(), header.Number[types.QuaiNetworkContext].Uint64(), []*big.Int{new(big.Int).Add(header.Difficulty[types.QuaiNetworkContext], blockchain.GetTdByHash(header.ParentHash[types.QuaiNetworkContext])[0]), new(big.Int).Add(header.Difficulty[types.QuaiNetworkContext], blockchain.GetTdByHash(header.ParentHash[types.QuaiNetworkContext])[1]), new(big.Int).Add(header.Difficulty[types.QuaiNetworkContext], blockchain.GetTdByHash(header.ParentHash[types.QuaiNetworkContext])[2])})
...
func TestLastBlock(t *testing.T) {
	...
	blocks := makeBlockChain(blockchain.CurrentBlock(), 1, blake3.NewFaker(), blockchain.db, 0)
...
func testExtendCanonical(t *testing.T, full bool) {
	...
	_, processor, err := newCanonical(blake3.NewFaker(), length, full)
	if err != nil {
	...
	better := func(td1, td2 []*big.Int) {
		if td2[types.QuaiNetworkContext].Cmp(td1[types.QuaiNetworkContext]) <= 0 {
...
func testBrokenChain(t *testing.T, full bool) {
	...
	db, blockchain, err := newCanonical(blake3.NewFaker(), 10, full)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.CurrentBlock(), 5, blake3.NewFaker(), db, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.CurrentHeader(), 5, blake3.NewFaker(), db, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool) {
	...
	db, blockchain, err := newCanonical(blake3.NewFaker(), 0, full)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	...
	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), blake3.NewFaker(), db, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), blake3.NewFaker(), db, len(second), func(i int, b *BlockGen) {
	...
	if full {
		...
		for header := blockchain.GetHeaderByNumber(blockchain.CurrentHeader().Number[types.QuaiNetworkContext].Uint64() - 1); header.Number[types.QuaiNetworkContext].Uint64() != 0; prev, header = header, blockchain.GetHeaderByNumber(header.Number[types.QuaiNetworkContext].Uint64()-1) {
	...
	want := []*big.Int{new(big.Int).Add(blockchain.genesisBlock.Difficulty(), big.NewInt(td)), new(big.Int).Add(blockchain.genesisBlock.Difficulty(), big.NewInt(td)), new(big.Int).Add(blockchain.genesisBlock.Difficulty(), big.NewInt(td))}
	if full {
		if have := blockchain.GetTdByHash(blockchain.CurrentBlock().Hash()); have[types.QuaiNetworkContext].Cmp(want[types.QuaiNetworkContext]) != 0 {
			t.Errorf("total difficulty mismatch: have %v, want %v", have, want)
		}
	} else {
		if have := blockchain.GetTdByHash(blockchain.CurrentHeader().Hash()); have[types.QuaiNetworkContext].Cmp(want[types.QuaiNetworkContext]) != 0 {
			t.Errorf("total difficulty mismatch: have %v, want %v", have, want)
		}
	}
}
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	headers := makeHeaderChain(blockchain.CurrentHeader(), 4, blake3.NewFaker(), db, 10)
	blocks := makeBlockChain(blockchain.CurrentBlock(), 4, blake3.NewFaker(), db, 10)
	...
	ncm, err := NewBlockChain(blockchain.db, nil, blockchain.chainConfig, blake3.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
	...
	if full {
		...
		if blocks[2].Header().GasLimit[types.QuaiNetworkContext] != ncm.GasLimit() {
...
// These tests cannot run until NewFakeFailer is implemented
/*
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			blocks := makeBlockChain(blockchain.CurrentBlock(), i, blake3.NewFaker(), db, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.engine = blake3.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.CurrentHeader(), i, blake3.NewFaker(), db, 0)

			failAt = rand.Int() % len(headers)
			failNum = headers[failAt].Number[types.QuaiNetworkContext].Uint64()
...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, blake3.NewFaker(), gendb, 1024, func(i int, block *BlockGen) {
		block.SetCoinbase(common.Address{0x00})

		// If the block number is multiple of 3, send a few bonus transactions to the miner
		if i%3 == 2 {
			for j := 0; j < i%4+1; j++ {
				tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, block.header.BaseFee[types.QuaiNetworkContext], nil), signer, key)
				if err != nil {
					panic(err)
				}
				block.AddTx(tx)
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec.Config, blake3.NewFaker(), vm.Config{}, nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec.Config, blake3.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec.Config, blake3.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
	...
	for i := 0; i < len(blocks); i++ {
		...
		if ftd, atd := fast.GetTdByHash(hash), archive.GetTdByHash(hash); ftd[types.QuaiNetworkContext].Cmp(atd[types.QuaiNetworkContext]) != 0 {
			t.Errorf("block #%d [%x]: td mismatch: fastdb %v, archivedb %v", num, hash, ftd, atd)
		}
		if antd, artd := ancient.GetTdByHash(hash), archive.GetTdByHash(hash); antd[types.QuaiNetworkContext].Cmp(artd[types.QuaiNetworkContext]) != 0 {
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	height := uint64(1024)
	blocks, receipts := GenerateChain(gspec.Config, genesis, blake3.NewFaker(), gendb, int(height), nil)
	...
	assert := func(t *testing.T, kind string, chain *BlockChain, header uint64, fast uint64, block uint64) {
		...
		if num := chain.CurrentHeader().Number[types.QuaiNetworkContext].Uint64(); num != header {
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec.Config, blake3.NewFaker(), vm.Config{}, nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	defer delfn()
	light, _ := NewBlockChain(lightDb, nil, gspec.Config, blake3.NewFaker(), vm.Config{}, nil, nil)
...
func TestChainTxReorgs(t *testing.T) {
	var (
		...
		gspec   = &Genesis{
			Config:   params.TestChainConfig,
			GasLimit: []uint64{3141592, 3141592, 3141592},
			Alloc: GenesisAlloc{
	...
	chain, _ := GenerateChain(gspec.Config, genesis, blake3.NewFaker(), db, 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastDrop, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr2), addr2, big.NewInt(1000), params.TxGas, gen.header.BaseFee[types.QuaiNetworkContext], nil), signer, key2)

			gen.AddTx(pastDrop)  // This transaction will be dropped in the fork from below the split point
			gen.AddTx(postponed) // This transaction will be postponed till block #3 in the fork

		case 2:
			freshDrop, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr2), addr2, big.NewInt(1000), params.TxGas, gen.header.BaseFee[types.QuaiNetworkContext], nil), signer, key2)
	...
	chain, _ = GenerateChain(gspec.Config, genesis, blake3.NewFaker(), db, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			pastAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr3), addr3, big.NewInt(1000), params.TxGas, gen.header.BaseFee[types.QuaiNetworkContext], nil), signer, key3)
		...
		case 2:
			...
			freshAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr3), addr3, big.NewInt(1000), params.TxGas, gen.header.BaseFee[types.QuaiNetworkContext], nil), signer, key3)
			gen.AddTx(freshAdd) // This transaction will be added exactly at reorg time

		case 3:
			futureAdd, _ = types.SignTx(types.NewTransaction(gen.TxNonce(addr3), addr3, big.NewInt(1000), params.TxGas, gen.header.BaseFee[types.QuaiNetworkContext], nil), signer, key3)
...
func TestLogReorgs(t *testing.T) {
	...
	chain, _ := GenerateChain(params.TestChainConfig, genesis, blake3.NewFaker(), db, 2, func(i int, gen *BlockGen) {
		if i == 1 {
			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), 1000000, gen.header.BaseFee[types.QuaiNetworkContext], code), signer, key1)
			if err != nil {
	...
	chain, _ = GenerateChain(params.TestChainConfig, genesis, blake3.NewFaker(), db, 3, func(i int, gen *BlockGen) {})
...
func TestCanonicalBlockRetrieval(t *testing.T) {
	...
	chain, _ := GenerateChain(blockchain.chainConfig, blockchain.genesisBlock, blake3.NewFaker(), blockchain.db, 10, func(i int, gen *BlockGen) {})
...
func TestEIP155Transition(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, blake3.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
	...
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], blake3.NewFaker(), db, 4, func(i int, block *BlockGen) {
		var (
			tx      *types.Transaction
			err     error
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
func TestEIP161AccountRemoval(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, blake3.NewFaker(), db, 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.LatestSigner(gspec.Config)
...
func TestBlockchainHeaderchainReorgConsistency(t *testing.T) {
	...
	(&Genesis{BaseFee: []*big.Int{big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee)}}).MustCommit(diskdb)
...
func TestBlockchainRecovery(t *testing.T) {
	...
	if num := ancient.CurrentHeader().Number[types.QuaiNetworkContext].Uint64(); num != midBlock.NumberU64() {
...
func TestInsertReceiptChainRollback(t *testing.T) {
	...
	gspec.MustCommit(ancientDb)
	ancientChain, _ := NewBlockChain(ancientDb, nil, gspec.Config, blake3.NewFaker(), vm.Config{}, nil, nil)
...
func TestLowDiffLongChain(t *testing.T) {
	...
	for number := head.NumberU64(); number > 0; number-- {
		...
		header = chain.GetHeader(header.ParentHash[types.QuaiNetworkContext], number-1)
	}
}
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	(&Genesis{BaseFee: []*big.Int{big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee)}}).MustCommit(chaindb)
...
func TestReorgToShorterRemovesCanonMapping(t *testing.T) {
	...
	if headerByNum := chain.GetHeaderByNumber(canonNum); headerByNum != nil {
		t.Errorf("expected header to be gone: %v", headerByNum.Number[types.QuaiNetworkContext].Uint64())
	}
}
...
func TestReorgToShorterRemovesCanonMappingHeaderChain(t *testing.T) {
	...
	canonNum := chain.CurrentHeader().Number[types.QuaiNetworkContext].Uint64()
...
func TestTransactionIndices(t *testing.T) {
	...
	height := uint64(128)
	blocks, receipts := GenerateChain(gspec.Config, genesis, blake3.NewFaker(), gendb, int(height), func(i int, block *BlockGen) {
		tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, block.header.BaseFee[types.QuaiNetworkContext], nil), signer, key)
		if err != nil {
			panic(err)
		}
	...
	blocks2, _ := GenerateChain(gspec.Config, blocks[len(blocks)-1], blake3.NewFaker(), gendb, 10, nil)
	...
	l := uint64(0)
	chain, err := NewBlockChain(ancientDb, nil, params.TestChainConfig, blake3.NewFaker(), vm.Config{}, nil, &l)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	headers := make([]*types.Header, len(blocks))
...
func benchmarkLargeNumberOfValueToNonexisting(b *testing.B, numTxs, numBlocks int, recipientFn func(uint64) common.Address, dataFn func(uint64) []byte) {
	var (
		...
		gspec           = Genesis{
			...
			GasLimit: []uint64{100e6, 100e6, 100e6}, // 100 M
		}
	)
	// Generate the original common chain segment and the two competing forks
	engine := blake3.NewFaker()
...
func TestDeleteCreateRevert(t *testing.T) {
	...
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 1, func(i int, b *BlockGen) {
		...
		tx, _ := types.SignTx(types.NewTransaction(0, aa,
			big.NewInt(0), 50000, b.header.BaseFee[types.QuaiNetworkContext], nil), types.HomesteadSigner{}, key)
		b.AddTx(tx)
		// One transaction to BBBB
		tx, _ = types.SignTx(types.NewTransaction(1, bb,
			big.NewInt(0), 100000, b.header.BaseFee[types.QuaiNetworkContext], nil), types.HomesteadSigner{}, key)
		b.AddTx(tx)
	})
	// Import the canonical chain
	diskdb := rawdb.NewMemoryDatabase()
...
func TestDeleteRecreateAccount(t *testing.T) {
	...
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 1, func(i int, b *BlockGen) {
		...
		tx, _ = types.SignTx(types.NewTransaction(1, aa,
			big.NewInt(1), 100000, b.header.BaseFee[types.QuaiNetworkContext], nil), types.HomesteadSigner{}, key)
		b.AddTx(tx)
	})
	// Import the canonical chain
	diskdb := rawdb.NewMemoryDatabase()
...
func TestEIP2718Transition(t *testing.T) {
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, engine, db, 1, func(i int, b *BlockGen) {
		...
		tx, _ := types.SignNewTx(key, signer, &types.AccessListTx{
			...
			To:       &aa,
			Gas:      30000,
			GasPrice: b.header.BaseFee[types.QuaiNetworkContext],
