...
func CreateConsensusEngine(stack *node.Node, chainConfig *params.ChainConfig, config *randomx.Config, notify []string, noverify bool, db ethdb.Database) consensus.Engine {
	...
	}, []byte(randomx.RxTestKey), notify, noverify)
	if nil != err {
		log.Fatal(err)
	}
	return engine
