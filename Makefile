...
ifeq (,$(wildcard nodelogs))
	...
	@nohup $(ZONE_CMD) --metrics --metrics.influxdb --metrics.influxdb.endpoint "http://0.0.0.0:8086" --metrics.influxdb.username "user" --metrics.influxdb.password "password" --metrics.influxdb.database "quai" >> $(ZONE_LOG_FILE) 2>&1 &
