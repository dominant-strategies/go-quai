...
var (
	...
	unknownBlock = types.NewBlock(&types.Header{
		GasLimit: []uint64{params.GenesisGasLimit, params.GenesisGasLimit, params.GenesisGasLimit},
		BaseFee:  []*big.Int{big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee)}}, nil, nil, nil, trie.NewStackTrie(nil))
)
...
func (f *fetcherTester) chainHeight() uint64 {
	...
	if f.fetcher.light {
		return f.headers[f.hashes[len(f.hashes)-1]].Number[types.QuaiNetworkContext].Uint64()
	}
...
func (f *fetcherTester) insertHeaders(headers []*types.Header) (int, error) {
	...
	for i, header := range headers {
		...
		if _, ok := f.headers[header.ParentHash[types.QuaiNetworkContext]]; !ok {
			return i, errors.New("unknown parent")
		}
		// Discard any new blocks if the same height already exists
		if header.Number[types.QuaiNetworkContext].Uint64() <= f.headers[f.hashes[len(f.hashes)-1]].Number[types.QuaiNetworkContext].Uint64() {
			return i, nil
		}
...
func TestQueueGapFill(t *testing.T) {
	...
	tester.fetcher.Enqueue("valid", blocks[hashes[skip]], []*types.ExternalBlock{})
	verifyImportCount(t, imported, len(hashes)-1)
	verifyChainHeight(t, tester, uint64(len(hashes)-1))
}
...
func TestImportDeduplication(t *testing.T) {
	...
	tester.fetcher.insertChain = func(blocks types.Blocks, extBlocks []*types.ExternalBlock) (int, error) {
		...
		return tester.insertChain(blocks, extBlocks)
	}
	...
	tester.fetcher.Enqueue("valid", blocks[hashes[1]], []*types.ExternalBlock{})
...
func TestDistantPropagationDiscarding(t *testing.T) {
	...
	tester.fetcher.Enqueue("lower", blocks[hashes[low]], []*types.ExternalBlock{})
	time.Sleep(10 * time.Millisecond)
	if !tester.fetcher.queue.Empty() {
		t.Fatalf("fetcher queued stale block")
	}
	// Ensure that a block with a higher number than the threshold is discarded
	tester.fetcher.Enqueue("higher", blocks[hashes[high]], []*types.ExternalBlock{})
	time.Sleep(10 * time.Millisecond)
	if !tester.fetcher.queue.Empty() {
...
func TestBlockMemoryExhaustionAttack(t *testing.T) {
	...
	for _, block := range attack {
		tester.fetcher.Enqueue("attacker", block, []*types.ExternalBlock{})
	}
	...
	for i := 0; i < maxQueueDist-1; i++ {
		tester.fetcher.Enqueue("valid", blocks[hashes[len(hashes)-3-i]], []*types.ExternalBlock{})
	}
	...
	tester.fetcher.Enqueue("valid", blocks[hashes[len(hashes)-2]], []*types.ExternalBlock{})
	verifyImportCount(t, imported, maxQueueDist)

	// Insert the remaining blocks in chunks to ensure clean DOS protection
	for i := maxQueueDist; i < len(hashes)-1; i++ {
		tester.fetcher.Enqueue("valid", blocks[hashes[len(hashes)-2-i]], []*types.ExternalBlock{})
		verifyImportEvent(t, imported, true)
	}
	verifyImportDone(t, imported)
}
