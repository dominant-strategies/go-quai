...
func (g *quairpc) killAndWait() {
	g.quai.Kill()
	g.quai.WaitExit()
}

func (g *quairpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.quai.Fatalf("callRPC %v: %v", method, err)
	}
}
...
func (g *quairpc) addPeer(peer *gethrpc) {
	...
	if err != nil {
		g.quai.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.quai.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.quai.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.quai.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *quairpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *quairpc) waitSynced() {
	...
	if err != nil {
		g.quai.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.quai.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.quai.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.quai.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.quai.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startQuaiWithIpc(t *testing.T, name string, args ...string) *quairpc {
	ipcName := fmt.Sprintf("quai-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	ipcpath := ipcEndpoint(ipcName, g.quai.Datadir)
	// We can't know exactly how long quai will take to start, so we try 10
...
func initQuai(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing quai: %v ", args)
	g := runQuai(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *quairpc {
	datadir := initQuai(t)
	t.Logf("Importing keys to quai")
	runQuai(t, "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startQuaiWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *quairpc {
	datadir := initQuai(t)
	return startQuaiWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
