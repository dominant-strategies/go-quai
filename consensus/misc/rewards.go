...
func CalculateReward(header *types.WorkObjectHeader, minerDifficulty *big.Int, exchangeRate *big.Int) *big.Int {
	var reward *big.Int
	if header.PrimaryCoinbase().IsInQiLedgerScope() {
		reward = new(big.Int).Set(CalculateQiReward(header, minerDifficulty))
	} else {
		reward = new(big.Int).Set(CalculateQuaiReward(header, minerDifficulty, exchangeRate))
	}
...
func QiToQuai(block *types.WorkObject, exchangeRate *big.Int, qiAmt *big.Int) *big.Int {
	quaiByQi := new(big.Int).Mul(CalculateQuaiReward(block.WorkObjectHeader(), block.MinerDifficulty(), exchangeRate), qiAmt)
	return new(big.Int).Quo(quaiByQi, CalculateQiReward(block.WorkObjectHeader(), block.MinerDifficulty()))
}
...
func QuaiToQi(block *types.WorkObject, exchangeRate *big.Int, quaiAmt *big.Int) *big.Int {
	qiByQuai := new(big.Int).Mul(CalculateQiReward(block.WorkObjectHeader(), block.MinerDifficulty()), quaiAmt)
	return new(big.Int).Quo(qiByQuai, CalculateQuaiReward(block.WorkObjectHeader(), block.MinerDifficulty(), exchangeRate))
}
...
func CalculateQuaiReward(header *types.WorkObjectHeader, minerDifficulty *big.Int, exchangeRate *big.Int) *big.Int {
	numerator := new(big.Int).Mul(exchangeRate, LogBig(minerDifficulty))
...
func CalculateQiReward(header *types.WorkObjectHeader, minerDifficulty *big.Int) *big.Int {
	qiReward := new(big.Int).Quo(minerDifficulty, params.OneOverKqi(header.NumberU64()))
