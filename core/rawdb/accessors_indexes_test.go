...
func TestLookupStorage(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			block := types.NewBlock(&types.Header{Number: []*big.Int{big.NewInt(314), big.NewInt(314), big.NewInt(314)}}, txs, nil, nil, newHasher())
...
func TestDeleteBloomBits(t *testing.T) {
	...
	for i := uint(0); i < 2; i++ {
		for s := uint64(0); s < 2; s++ {
			WriteBloomBits(db, i, s, params.MainnetPrimeGenesisHash, []byte{0x01, 0x02})
	...
	check(0, 0, params.MainnetPrimeGenesisHash, false)
	check(0, 0, params.RinkebyGenesisHash, false)
	check(0, 1, params.MainnetPrimeGenesisHash, false)
	check(0, 1, params.RinkebyGenesisHash, false)

	// Bit1 shouldn't be affect.
	check(1, 0, params.MainnetPrimeGenesisHash, true)
	check(1, 0, params.RinkebyGenesisHash, true)
	check(1, 1, params.MainnetPrimeGenesisHash, true)
