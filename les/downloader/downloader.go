		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				if mode != LightSync {
					...
					if !gotHeaders && td[types.QuaiNetworkContext].Cmp(d.blockchain.GetTd(head.Hash(), head.NumberU64())) > 0 {
				...
				if mode == FastSync || mode == LightSync {
					...
					if td[types.QuaiNetworkContext].Cmp(d.lightchain.GetTd(head.Hash(), head.Number[types.QuaiNetworkContext].Uint64())) > 0 {
						return errStallingPeer
					}
				}
