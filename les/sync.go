...
func (h *clientHandler) synchronise(peer *serverPeer) {
	...
	if currentTd != nil && peer.Td()[types.QuaiNetworkContext].Cmp(currentTd[types.QuaiNetworkContext]) < 0 {
		return
	}
	...
	case latest.Number[types.QuaiNetworkContext].Uint64() >= (checkpoint.SectionIndex+1)*h.backend.iConfig.ChtSize-1:
		mode = lightSync
