...
type PeerManager interface {
	...
	GetPeers(topic string, quality PeerQuality) []p2p.PeerID
...
func (pm *BasicPeerManager) GetPeers(topic string, quality PeerQuality) []p2p.PeerID {
...
func (pm *BasicPeerManager) queryDHT(topic string, peerList []p2p.PeerID, peerCount int) []p2p.PeerID {
...
func (pm *BasicPeerManager) getBestPeersWithFallback(topic string) []p2p.PeerID {
	...
	if bestPeersCount < C_peerCount {
		...
		bestPeerList = append(bestPeerList, pm.getResponsivePeersWithFallback(topic)...)
		return bestPeerList
	}
	return pm.getPeersHelper(pm.peerDBs[topic][Best], C_peerCount)
}

func (pm *BasicPeerManager) getResponsivePeersWithFallback(topic string) []p2p.PeerID {
	...
	if responsivePeersCount < C_peerCount {
		...
		responsivePeerList = append(responsivePeerList, pm.getLastResortPeers(topic)...)
...
func (pm *BasicPeerManager) getLastResortPeers(topic string) []p2p.PeerID {
