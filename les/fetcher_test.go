...
func verifyChainHeight(t *testing.T, fetcher *lightFetcher, height uint64) {
	local := fetcher.chain.CurrentHeader().Number[types.QuaiNetworkContext].Uint64()
	if local != height {
...
func testSequentialAnnouncements(t *testing.T, protocol int) {
	...
	for i := uint64(1); i <= s.backend.Blockchain().CurrentHeader().Number[types.QuaiNetworkContext].Uint64(); i++ {
...
func testGappedAnnouncements(t *testing.T, protocol int) {
	...
	hash, number := latest.Hash(), latest.Number[types.QuaiNetworkContext].Uint64()
	td := rawdb.ReadTd(s.db, hash, number)
	...
	blocks, _ := core.GenerateChain(rawdb.ReadChainConfig(s.db, s.backend.Blockchain().Genesis().Hash()), s.backend.Blockchain().GetBlockByNumber(3),
		blake3.NewFaker(), s.db, 2, func(i int, gen *core.BlockGen) {
	...
	check := func(height []uint64, expected uint64, callback func()) {
		for i := 0; i < len(height); i++ {
			for j := 0; j < len(servers); j++ {
				...
				hash, number := h.Hash(), h.Number[types.QuaiNetworkContext].Uint64()
