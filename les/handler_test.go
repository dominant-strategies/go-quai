...
import (
	...
	"github.com/spruce-solutions/go-quai/les/downloader"
...
func testGetStaleCode(t *testing.T, protocol int) {
	...
	check(bc.CurrentHeader().Number[types.QuaiNetworkContext].Uint64(), [][]byte{testContractCodeDeployed}) // Fresh contract
}
...
func testGetProofs(t *testing.T, protocol int) {
	...
	for i := uint64(0); i <= bc.CurrentBlock().NumberU64(); i++ {
		header := bc.GetHeaderByNumber(i)
		trie, _ := trie.New(header.Root[types.QuaiNetworkContext], trie.NewDatabase(server.db))
...
func testGetStaleProof(t *testing.T, protocol int) {
	...
	check := func(number uint64, wantOK bool) {
		...
		if wantOK {
			proofsV2 := light.NewNodeSet()
			t, _ := trie.New(header.Root[types.QuaiNetworkContext], trie.NewDatabase(server.db))
	...
	check(bc.CurrentHeader().Number[types.QuaiNetworkContext].Uint64(), true) // Fresh proof
}
...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), blake3.NewFaker(), server.db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), blake3.NewFaker(), server.db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
