...
import (
	...
	"github.com/spruce-solutions/go-quai/les/fetcher"
...
func (fp *fetcherPeer) forwardAnno(td *big.Int) []*announce {
	...
	for ; cutset < len(fp.announcesList); cutset++ {
		...
		if anno.data.Td[types.QuaiNetworkContext].Cmp(td) > 0 {
			break
		}
...
func (f *lightFetcher) mainloop() {
	...
	for {
		...
		case anno := <-f.announceCh:
			...
			if peer.latest != nil && data.Td[types.QuaiNetworkContext].Cmp(peer.latest.Td[types.QuaiNetworkContext]) <= 0 {
			...
			if localTd != nil && data.Td[types.QuaiNetworkContext].Cmp(localTd[types.QuaiNetworkContext]) <= 0 {
				continue
			}
			peer.addAnno(anno)
			...
			f.forEachPeer(func(id enode.ID, p *fetcherPeer) bool {
				...
				for _, anno := range removed {
					if header := f.chain.GetHeaderByHash(anno.data.Hash); header != nil {
						...
						if td != nil && td[types.QuaiNetworkContext].Cmp(anno.data.Td[types.QuaiNetworkContext]) != 0 {
							droplist = append(droplist, id)
							break
						}
					}
